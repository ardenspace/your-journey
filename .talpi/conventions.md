...
- step 4: 알림 탭 리스너(`addCapsuleNotificationTapListener` — DB 조회 후 없는 일기는 조용히 홈, expo-notifications plugin 등록). ⚠️ cold-start 탭(앱 종료 상태에서 알림 탭) 라우팅은 미보장 — `getLastNotificationResponseAsync` 경로 필요, Phase 4 마무리에서 처리
